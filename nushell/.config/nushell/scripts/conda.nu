...
def --env load-conda-info-env [] {
    ...
    if (not (has-env CONDA_INFO)) {
        export-env {
            ...
            $env.CONDA_INFO = if ($cmd_base == "conda") {
                ...
                # No command found
                print --stderr "WARN: No conda command found."
                null
            }
        } # End export-env
...
export def --env activate [
    ...
    if ($conda_info == null) {
        print --stderr "Error: Conda info could not be loaded. Cannot activate."
        return
    }
    ...
    if ($conda_info.envs_dirs == null) or ($conda_info.root_prefix == null) or ($conda_info.envs == null) {
         print --stderr "Error: Failed to load essential Conda info (envs_dirs, root_prefix, envs). Cannot activate."
         return
    }
